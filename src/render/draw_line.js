...
export default function drawLine(
    ...
    if (gradient) {
        ...
        if (!gradientTexture)
            gradientTexture = layer.gradientTexture = new Texture(
                context,
                layer.gradient,
                gl.RGBA
            );
    ...
    for (const coord of coords) {
        ...
        if (constantPattern && tile.imageAtlas) {
            ...
            const posFrom =
                tile.imageAtlas.patternPositions[constantPattern.from];
            if (posTo && posFrom)
                programConfiguration.setConstantPatternPositions(
                    posTo,
                    posFrom
                );
        }
