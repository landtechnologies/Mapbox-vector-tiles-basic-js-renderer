...
const shaders: {
    [string]: { fragmentSource: string, vertexSource: string }
} = {
    prelude: {
    ...
    lineGradient: {
        fragmentSource: fs.readFileSync(
            __dirname + "/../shaders/line_gradient.fragment.glsl",
            "utf8"
        ),
        vertexSource: fs.readFileSync(
            __dirname + "/../shaders/line_gradient.vertex.glsl",
            "utf8"
        )
    },
    linePattern: {
...
for (const programName in shaders) {
    ...
    program.fragmentSource = program.fragmentSource.replace(
        re,
        (
            match: string,
            operation: string,
...
`;
    ...
    program.vertexSource = program.vertexSource.replace(
        re,
        (
            match: string,
            operation: string,
